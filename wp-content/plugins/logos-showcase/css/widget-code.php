...

{
	...
	function widget($args, $instance)
	{
		...
		echo build_lshowcase($orderby, $category, $activeurl, $style, $interface, $tooltip, $description, $limit, $slidersettings,$img);
		echo $after_widget;
	}
...
?>
 ...
 <p>
     <label for="<?php
		echo $this->get_field_id( 'description' ); ?>">Show Info Below:</label>
        <br />
        <select id="<?php
		echo $this->get_field_id( 'description' ); ?>" name="<?php
		echo $this->get_field_name( 'description' ); ?>">
          <option value="true" <?php
		selected($description, 'true' ); ?>>Yes - Title</option>
 		
 		<option value="true-description" <?php
		selected($description, 'true-description' ); ?>>Yes - Description</option>
